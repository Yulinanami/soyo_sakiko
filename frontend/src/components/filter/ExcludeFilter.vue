...
<template>
  <div v-if="props.open" class="flex items-center gap-3 flex-wrap">
    ...
    <div class="flex gap-1.5 flex-wrap">
      ...
      ]" @click="toggleTag(tag)" :title="isSelected(tag) ? '点击取消排除' : '点击排除此标签'">
        {{ tag }}
      </button>
    </div>
    <div class="flex gap-1">
      <input v-model="newTag" type="text" placeholder="添加排除..." @keyup.enter="addCustomTag"
      ...
      <button @click="addCustomTag" :disabled="!newTag.trim()" class="w-8 h-8 border border-gray-200 bg-white rounded-full cursor-pointer text-lg flex items-center justify-center text-gray-500
