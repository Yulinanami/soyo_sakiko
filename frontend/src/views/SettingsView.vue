  ...
  <div class="min-h-screen bg-gray-50">
    <header class="bg-white border-b border-gray-200">
      <div class="px-6 py-6">
        ...
        <p class="text-sm text-gray-500 mt-1">登录并配置需要凭证的数据源。</p>
      </div>
    </header>

    <main class="p-6">
      <section class="bg-white rounded-xl border border-gray-200 p-6 space-y-4 max-w-3xl">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-base font-semibold text-gray-900">Pixiv 登录</div>
            <div class="text-sm text-gray-500">用于搜索 Pixiv 同人文。</div>
          </div>
          <div class="flex items-center gap-2">
            <button
              type="button"
              class="flex items-center gap-2 px-4 py-2 border-2 rounded-lg text-sm transition-all"
              :class="credentialStatus.pixiv.configured ? 'border-green-400 text-green-700' : 'border-gray-200 hover:border-sakiko hover:text-sakiko-dark'"
            ...
            <button
              type="button"
              class="px-3 py-2 border-2 rounded-lg text-sm border-red-200 text-red-600 hover:border-red-400 hover:text-red-700 transition-all"
        ...
        <div class="text-xs text-gray-500" v-if="credentialStatus.pixiv.message">
          {{ credentialStatus.pixiv.message }}
        </div>
      </section>

      <section class="bg-white rounded-xl border border-gray-200 p-6 space-y-4 max-w-3xl mt-6">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-base font-semibold text-gray-900">Lofter 登录</div>
            <div class="text-sm text-gray-500">用于搜索 Lofter 同人文。</div>
          </div>
          <div class="flex items-center gap-2">
            <button
              type="button"
              class="flex items-center gap-2 px-4 py-2 border-2 rounded-lg text-sm transition-all"
              :class="credentialStatus.lofter.configured ? 'border-green-400 text-green-700' : 'border-gray-200 hover:border-soyo hover:text-soyo-dark'"
        ...
        <div class="text-xs text-gray-500" v-if="credentialStatus.lofter.message">
